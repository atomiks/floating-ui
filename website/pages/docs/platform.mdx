...
from anywhere, and the library will perform the right
...
The `x` and `y` values of a reference `Rect` should be its
coordinates relative to the floating element's `offsetParent`
element, not the viewport.

#### floating

Both `x` and `y` are not relevant initially, so you can set these
both of these to `0`.
