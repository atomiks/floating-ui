...
tippy('#myButton', {
  ...
  animation: 'fade',
  trigger: 'click',
  plugins: [animation, aria, dismiss],
});
```
...
popover via `esc`, import the `aria` and `dismiss` plugins:
...
you don't need to pass all the plugins and props every time you
want to add a popover to your website.
...
`dataAttributes` plugin to reduce this to a single call:
...
export function popover(targets, options = {}) {
  return tippy(targets, {
    ...
    trigger: options.trigger ?? 'click',
    interactive: true,
    plugins: [animation, aria, dismiss, dataAttributes],
  });
}
```
