...
`staticSide{:js}` depends on the `placement` that gets chosen.
For instance, if the placement is `'bottom'{:js}`, then we want
the arrow to be positioned 4px outside of the **top** of the
tooltip (so we use a negative number).
