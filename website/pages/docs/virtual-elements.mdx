...
`getBoundingClientRect` property, which mimics a real element's
one:
...
`getBoundingClientRect` data is **derived** from a real element's
one. For instance, you're modifying a real reference element's
`getBoundingClientRect` in some way, and using a virtual element
enables this.
